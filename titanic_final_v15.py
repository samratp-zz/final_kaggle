...
def fill_missing_age(missing_age_train, missing_age_test):
    ...
    gbm_reg_grid = model_selection.GridSearchCV(gbm_reg, gbm_reg_param_grid, cv = 10, n_jobs = 15, verbose = 1, scoring = 'neg_mean_squared_error')
    ...
    lrf_reg_grid = model_selection.GridSearchCV(lrf_reg, lrf_reg_param_grid, cv = 10, n_jobs = 15, verbose = 1, scoring = 'neg_mean_squared_error')
    ...
    missing_age_test['Age'] = missing_age_test[['Age_GB', 'Age_LRF']].mean(axis = 1).round().astype(int)
...
def get_top_n_features(titanic_train_data_X, titanic_train_data_y, top_n_features):
    ...
    rf_param_grid = {'n_estimators' : [750], 'max_depth':[15], 'min_samples_split':[15], 'min_samples_leaf':[1]}
    rf_grid = model_selection.GridSearchCV(rf_est, rf_param_grid, n_jobs = 35, cv = 10, verbose = 1)
    ...
    print(str(features_top_n_rf[:50]))

    et_est = ensemble.ExtraTreesClassifier(random_state = 42)
    et_param_grid = {'n_estimators' : [750], 'min_samples_split':[10], 'min_samples_leaf':[2], 'max_depth':[30]}
    et_grid = model_selection.GridSearchCV(et_est, et_param_grid, n_jobs = 35, cv = 10, verbose = 1)
    ...
    print(str(features_top_n_et[:50]))
    
    #### Merge top_n_features from all three models
    features_top_n = pd.concat([features_top_n_rf, features_top_n_et], ignore_index = True).drop_duplicates()
...
#### Create Name_Length_Category
print(combined_train_test.groupby(['Survived', 'Name_Length'])['Survived'].count())
...
le_fare.fit(np.array(['Very_Short_Name', 'Short_Name', 'Medium_Name', 'Long_Name', 'Very_Long_Name']))
...
pclass_fare_cat_dummies_df = pd.get_dummies(combined_train_test['Pclass_Fare_Category'],
                                prefix = combined_train_test[['Pclass_Fare_Category']].columns[0])
combined_train_test = pd.concat([combined_train_test, pclass_fare_cat_dummies_df], axis = 1)
...
#### Ticket
combined_train_test['Ticket_Letter'] = combined_train_test['Ticket'].apply(lambda x: np.NaN if x.isnumeric() else x)
combined_train_test['Ticket_Number'] = combined_train_test['Ticket'].apply(lambda x: pd.to_numeric(x, errors='coerce'))
combined_train_test['Ticket_Number'].fillna(0, inplace = True)
combined_train_test = pd.get_dummies(combined_train_test, columns = ['Ticket_Letter', 'Ticket_Number'])
combined_train_test.shape
...
combined_train_test.drop(['Name', 'Ticket', 'PassengerId', 'Embarked', 'Sex', 'Title', 'Fare_Category', 'Family_Size_Category', 'Age_Category', 'First_Name', 'Last_Name', 'Original_Name', 'Name_Length_Category', 'Pclass_Fare_Category'], axis = 1, inplace = True)
...
et_est = ensemble.ExtraTreesClassifier(n_estimators = 750, max_depth = 35,  n_jobs = 15, criterion = 'entropy', min_samples_split = 3, min_samples_leaf = 1, random_state = 42, verbose = 1)
